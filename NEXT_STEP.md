...
## Where things stand (2026-08-31 end of session — big one, all pushed)

**Report-context feature (from 2026-08-28) is now fully confirmed working** — Ori walked the full test list live on-device through section 16; all three entry points (chat's Get Help → "Report {name}", item listing's "Report this listing", `PublicProfileScreen`'s own flag) correctly open the shared report modal with context, and the admin side (`AdminReportsScreen`'s context display, `AdminConversationViewScreen`) renders correctly. Two small UI bugs found during that walkthrough were fixed: the report modal's Cancel button was colliding with the Android nav bar (safe-area inset now added to its bottom padding), and it had no tap-outside-to-dismiss (added).

**Biometric gate removed entirely, spec 4.2 reinterpreted correctly.** Ori clarified that spec 4.2's "biometric verification" means real external identity verification (an ID-document/selfie KYC service, like a scooter-rental app checks you're a real person) — not an on-device Face ID/fingerprint app-unlock. The on-device gate built earlier under the wrong reading (`BiometricContext`, the read-only banner, the enforcement checks in `AddItemScreen`/`ItemDetailScreen`) is now **deleted entirely** — it verified the phone's owner, not the platform user's real-world identity, so it never actually satisfied the spec. Real KYC is a deliberate, deferred decision: Stripe Identity is the natural fit (~$1.50/verification, Stripe's already integrated) but needs a new edge function, a DB column + migration, a new native SDK + rebuild, and upload-flow UI — not something to add without discussing cost/timing with Ori first. Documented as a known deviation in `CLAUDE.md`.

**Phone verification — real bug found and fixed.** With the biometric gate gone, phone verification (spec's actual "prove this is a real person" mechanism) needed to actually work — it turned out `PhoneVerificationScreen.tsx`'s `sendCode()` was a complete stub (just flipped UI state, never called Supabase), and `verifyCode()`'s real OTP path never called `onVerified()` even on success. Both fixed; the `123456` dev bypass is now gated behind `__DEV__` instead of always being live.

**Full chat media feature shipped: photo, video, and voice messages in `ChatRoomScreen`.** New components `VoiceMessageBubble.tsx` (waveform, tap-to-seek, live countdown) and `VideoMessageBubble.tsx` (native controls via expo-video); a custom centered "Send Media" bottom-sheet modal (was misaligned right-side on Android with the old `Alert.alert`); tap-to-record voice notes with a live metering-based waveform. Real bugs found and fixed along the way, roughly in the order Ori hit them testing:
- A voice-bubble off-by-one lit the first waveform bar even at zero progress (`<=` → `<`).
- iOS played voice messages silently unless a recording had already run this session — fixed by setting `playsInSilentMode: true` at the top of every playback, not just after recording.
- Playback/seek were slow and occasionally landed wrong — was re-fetching a signed URL on every tap and reading a possibly-stale React `duration` instead of the live player's; both fixed (signed URLs now cached, seek reads the live player).
- iOS crashed the instant recording started — root-caused via a pulled crash log to a real upstream Expo bug (`expo/expo#43680`, `expo-modules-core`'s `EXPermissionsService.h` missing a method declaration implemented in the `.m`). Fixed by patching the header directly, locked in via `patch-package` (new `patches/` dir + `postinstall` script) so it survives `npm install`.
- Android's mic-permission dialog never appeared — root-caused to `expo-image-picker`'s `"microphonePermission": false` config silently stripping `RECORD_AUDIO` from the built manifest; removed that config key.

**Badge/unread notifications weren't updating live across devices — real DB-level bug, fixed.** Root-caused (not guessable from the UI) via direct Postgres inspection: `conversations`/`messages` lacked `REPLICA IDENTITY FULL`, which Realtime's Postgres Changes needs for an UPDATE event to pass RLS and actually reach subscribers — `transactions`/`purchases` (which always worked live) already had it. Fixed via migration.

**New general navigation feature: double-tap any tab to reset it to that tab's home screen — confirmed working.** This was needed because `ManageItemScreen`, reached cross-tab from a chat's overflow menu, could leave the Profile tab permanently stuck on it with no way back to the actual profile screen. Took several rounds to get right (see `MainTabNavigator.tsx`'s `screenListeners`/`findRouteByKey` for the final version) — the real root cause was that a tab's *very first* navigation this session, when reached via a cross-tab `navigate('Profile', {screen: 'ManageItem', params})` shortcut, lands as unresolved deep-link `params` directly on the tab route with no real nested stack at all, so there's nothing for a normal `CommonActions.reset` to target. The fix branches on both cases (reset the real nested stack when one exists; clear the unresolved params when it doesn't) and applies uniformly to every tab, not just this one screen. **Confirmed working live on the Galaxy** at the end of this session.
Also fixed in the same area: `ManageItemScreen`'s calendar → tapping an approved rental now correctly lands on the Deal Board tab with the right listing highlighted (was matching against the wrong row in `feedRows`); its own back button/gesture/hardware-back now always return to the originating chat conversation via a `returnToChat` param, never to Home/Profile; its calendar icon moved out of `ChatRoomScreen`'s header into the existing 3-dot overflow menu.

**New reusable pattern: Overflow Menu, documented in `CLAUDE.md` alongside SAS and Badge Jump.** `src/components/OverflowMenu.tsx` — a 3-dot header button for any screen's secondary actions, matching `ProfileScreen`'s existing hamburger-sheet look. Applied to `PublicProfileScreen` (Report user), `ItemDetailScreen` (Report this listing), and `ChatRoomScreen` (Report {name}, Manage Item Calendar) — each replacing a scattered standalone icon/link.

**Smaller fixes this session:**
- Swept `messages` for live profanity Ori typed while testing — fixed the message text in place (explicitly *not* deleted, to preserve real transaction/conversation history).
- Removed `PublicProfileScreen`'s "this lender has cancelled N confirmed rentals" warning banner entirely, per Ori's call — not a UI bug, a deliberate feature removal.

**Not committed/pushed yet as of this note being written — see "Immediate next steps."** iPhone was disconnected mid-session ("אני מנתק לך את האייפון, אתה תישאר רק עם גלקסי") — **all testing is Galaxy-only until further notice.**

### Immediate next steps, in order
1. Commit and push everything from this session (in progress).
2. Resume the older pending checklist below — testing owed (account deletion end-to-end, GDPR export Share sheet, Stripe Connect payout completing a full pickup→return cycle), the older pending features (dispute resolution redesign, `ReviewsListScreen`, overdue-card simplification, support-thread notifications in Chats), and the two dashboard actions that need Ori directly (enable the Supabase login-lockout hook, confirm the Google Maps quota/budget alert).

---

## Where things stood (2026-08-27 end of session — big one)
**Distributable Android APK now exists and is confirmed working — see "Android APK distribution" section below.** Native rebuild done on both devices (identifier changed twice — see that section for why). Also this session: duplicate `messageParty` extracted into `src/hooks/useMessageParty.ts`; a real bug fixed and shipped — `AdminOverdueScreen`/`AdminDisputesScreen`'s "Message Renter/Lender" buttons dead-ended in `admin_ensure_support_thread`'s "cannot open a support thread with another admin" error whenever that party happened to be an admin, fixed by exposing `renter_is_admin`/`lender_is_admin` and hiding the button client-side; a second real bug — `testAccounts.ts`'s unconditional import broke **any** clean-checkout build, not just distributable ones, fixed by moving those credentials to an env var; and AI auto-fill's entry point moved from a buried post-photo button to a prominent "Complete with AI" button at the top of Add Item, confirmed working end-to-end.
...
- **`eas.json`** added — `preview` profile, `internal` distribution, `apk` build type (direct download link, no Play Store). First-time `eas project:init` linked the repo to `@oriper/useit` on Expo's servers (`app.json`'s `extra.eas.projectId` now set).
- **The four `EXPO_PUBLIC_*` client vars** (Supabase URL/anon key, Stripe publishable key, Google Maps key) pushed to EAS's own environment-variable store via `eas env:set preview` — these are separate from the local `.env` file and from git; a cloud build doesn't see your local `.env` at all, so this step is required every time a new client-facing key is added, not just once.
- **Real bug found and fixed**: `ProfileScreen.tsx`'s top-level `import { TEST_ACCOUNTS } from '../config/testAccounts'` broke Metro bundling outright on any checkout without that gitignored file present — not just an EAS-specific quirk, this would have broken CI or any teammate's clean clone too. Root cause: Metro resolves imports statically at bundle time, so a gitignored (and thus, for EAS, archive-excluded) file can never be safely imported unconditionally, no matter how it's gated at the UI level. **Fix**: `src/config/testAccounts.ts` is now tracked in git with an always-empty default export; real credentials come from `EXPO_PUBLIC_TEST_ACCOUNTS_JSON` in the local, gitignored `.env` instead (same pattern already used for Supabase/Stripe/Maps keys) — this is deliberately **never** pushed to EAS's env store, so any EAS build (including this distributable one) gets an empty list, and the `__DEV__`-gated Switch User UI stays invisible in any release build with no manual "remember to delete the file" step needed anymore. `testAccounts.example.ts` deleted (obsolete — see `.env.example` instead).
- **First successful build**: `com.useitapp.app`, v1.0.0, confirmed installing and running correctly on the Galaxy (after uninstalling the locally-signed dev-client build first — same package name, different signing certificate, so Android refuses to install over it without a clean uninstall first; this isn't an Expo bug, it's normal Android signature-mismatch behavior).
- **Distribution links**: build page (has a scannable QR code behind the "Install" button, may require an Expo login to view) — `https://expo.dev/accounts/oriper/projects/useit/builds/156d24c3-e5f8-40e7-b40a-c8f73f504bf4`; direct `.apk` download (no login needed, this is the one to actually share) — `https://expo.dev/artifacts/eas/lpDxjbAcIVrfIu2L3zsOusr_gvk6AEuiY2gjWgYRI1Q.apk`. Note this specific link is tied to this specific build — a future rebuild (e.g. after more fixes before Demo Day) will need `eas build --profile preview --platform android` run again and the new link swapped in.
- **Still true**: iOS has no equivalent — see item 1 in the next-steps list above.

## ✅ NEW (2026-08-27) — AI auto-fill entry point moved to the top of Add Item
Was only discoverable after already adding a photo manually (a button appeared below Title/Category/Description/Price/City, easy to miss if you didn't already know the feature existed). Replaced with a single **"Complete with AI"** button at the very top of the form, above Title — taps straight into the camera, sets the result as the cover photo, and runs the analysis immediately. The old inline button (and its now-dead code) was removed entirely rather than keeping two entry points for the same action. Confirmed working end-to-end on the Galaxy.
...
- ~~**Biometric gate (spec 4.2)** — `expo-local-authentication`, checked once per session right after login...~~ **Removed entirely 2026-08-31** — this was built under a misread of spec 4.2 (it means real external KYC, not an on-device unlock). See the 2026-08-31 section at the top of this file.
...
**Done and confirmed, dropped from this list (2026-08-31):** report-context feature end-to-end, all three report entry points + admin side (`AdminReportsScreen` context, `AdminConversationViewScreen`) — walked live through section 16 of the test plan; chat photo/video/voice messages on Android including playback/seek; badge/notifications updating live across devices; double-tap-to-reset-tab on Android; `ManageItemScreen` calendar → Deal Board highlight + return-to-chat navigation.
...
- Phone verification's real OTP path (`sendCode`/`verifyCode`, fixed 2026-08-31) — only exercised via the `__DEV__` `123456` bypass so far, not with a real SMS code end-to-end.
- Everything iOS-specific from this session's chat-media/navigation work (photo/video/voice sending, the mic-permission crash fix, double-tap-to-reset) — **iPhone was disconnected mid-session and testing is Galaxy-only until it's reconnected.** Re-verify the whole chat-media feature and the double-tap fix on iPhone once it's back.
...
**Note (2026-08-31): the biometric-gate caveat that used to be here no longer applies** — `BiometricContext` was removed entirely (see the top-of-file 2026-08-31 section). Spec 4.2 is now understood to mean real external KYC, deliberately deferred, not an on-device unlock gate.
