...
## ✅ NEW (2026-08-26) — DB profanity cleanup + SwipeAndRent → UseIT rebrand
...
**Rebrand**: Ori wants zero trace of "SwipeAndRent" anywhere reviewers/Demo Day attendees see. "UseIT" is already the platform's own established in-universe brand for support/arbitration throughout the app (Contact UseIT, Message UseIT, "UseIT reviewed this dispute...", `admin_ensure_support_thread`) — the outer app name had just never been aligned with it. Split into safe-now vs needs-a-decision, same reasoning as the Google Maps key and Switch User fixes earlier:

- ✅ **Done (pure text, zero identifier/technical risk)**: app.json's display `"name"` and all 5 permission-prompt strings; in-app UI text (onboarding's "How will you use UseIT?", the GDPR export Share sheet title, the Stripe payment sheet's `merchantDisplayName` for both rental and purchase checkout); 4 edge functions' user-visible strings (Stripe charge descriptions in `create-payment-intent`/`admin-charge`/`charge-late-fee`, the "Returning to UseIT…" bounce page in `connect-return`) — **redeployed live** via the Supabase MCP tool, preserving each function's exact `verify_jwt` setting (`connect-return` stays unauthenticated by design, the other three stay JWT-required); README (title, alt text, overview paragraph, Project Status section); `package.json`'s internal `"name"` field; CLAUDE.md/AGENTS.md header lines (was already inconsistently "Swap&Rent" vs "SwipeAndRent" before this — now both say UseIT).
- ⚠️ **Deliberately NOT changed — needs Ori's decision, real risk right before Demo Day**:
  1. **app.json's `slug`/`scheme`/`bundleIdentifier`/`android.package`** (still `swipeandrent`/`com.swipeandrentapp`) — the `scheme` is load-bearing for OAuth redirects and the `connect-return` deep link (`swipeandrent://connect-return`); the bundle/package identifiers are the actual app identity on both devices' *already-trusted, currently-working* installs. Changing any of these means a new provisioning-trust flow on the iPhone and a fresh install (uninstall-then-reinstall, since Android treats a new package name as a different app) on the Galaxy, not just a rebuild — real risk of losing dev-tested state days before the demo. If Ori wants this too, budget time to redo both devices' trust flow afterward.
  2. **The GitHub repo name itself** (still `SwipeAndRent`) — not something changeable from here (no GitHub access this session). If Ori renames it via GitHub's own settings, update the README's `git clone` URL (currently `github.com/pamnati592/SwipeAndRent.git`, deliberately left matching the real repo location until it's renamed) and any local `git remote -v` on both dev machines to match.
- **Still says "SwipeAndRent" and deliberately left alone**: `product-spec.pdf` (the official course deliverable, not something to silently rewrite) and the native `ios/SwipeAndRent`/`android` folder paths referenced in the dev-environment notes below (gitignored, regenerated by `expo prebuild` — see the updated note next to the 7-day iPhone cert instructions).
...
- **iPhone builds expire after 7 days** (free Apple personal team). Re-run `npx expo run:ios --device <UDID>` on USB to refresh, then trust the new certificate on-device at Settings → General → VPN & Device Management (can't be automated). After any `expo prebuild`, re-run `plutil -remove aps-environment ios/SwipeAndRent/SwipeAndRent.entitlements` — prebuild re-adds a push entitlement the free team can't sign (app only uses local notifications). **Note (2026-08-26):** app.json's display `"name"` was renamed SwipeAndRent → UseIT (bundleIdentifier/slug/scheme deliberately left unchanged, see NEXT_STEP.md's rename entry below) — the next `expo prebuild` may regenerate the iOS folder under a new name to match; check the actual path under `ios/` before running this command rather than assuming it's still `ios/SwipeAndRent/...`.
