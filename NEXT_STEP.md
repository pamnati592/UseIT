...
### ⚠️ NEW (2026-08-26) — Google Maps API key missing, breaks city picker
Caught live on the Galaxy: onboarding's "Choose city" → "Use my current location" fails with `Could not detect city / Maps API key missing`. Root cause confirmed in `src/services/places.ts` — `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY` isn't set in `.env` at all (only Supabase/Stripe/Groq keys are). This is the same Google Cloud project referenced in backlog item L below ("Google Cloud account hardening") — so a key needs to be created from scratch, not just re-added; per Ori (2026-08-26), no key exists yet. Two distinct breakages from the same missing key:
- `reverseGeocodeToCity` (current-location button) **throws** with the visible error above.
- `autocompleteCities` (typing a city name) **fails silently** — it just returns `[]` on a missing key (`places.ts:27`), so typing shows no suggestions with no error at all. Worth tightening once the key exists, so a future regression doesn't hide the same way.
This blocks city selection anywhere `CityPicker` is used (onboarding, profile, item location) — **must be resolved before Demo Day**, since a fresh install can't get past onboarding's city step without it. Ori needs to create the key in Google Cloud Console (Places API + Geocoding API enabled, quota-capped per item L) and hand it over to get wired into `.env`. `.env.example` was also missing this var entirely despite the README listing it as required — added as a placeholder there now.
